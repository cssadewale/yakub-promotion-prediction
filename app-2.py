...
# The model file is stored on Google Drive because it exceeds GitHub's 25 MB
# file size limit. On the very first run, gdown downloads it automatically into
# the app's working directory and caches it. Every subsequent run loads it
# instantly from that local cache — no repeated downloads.

MODEL_PATH    = "best_random_forest_model.joblib"
DRIVE_FILE_ID = "1WpmmwWWSVrHBDoAa6AhySH0tc8zwSKOt"
DRIVE_URL     = f"https://drive.google.com/uc?id={DRIVE_FILE_ID}"

@st.cache_resource(show_spinner="Loading prediction model — please wait...")
def load_model():
...
if model is None:
    st.error(
        ...
        "The app tried to download the model from Google Drive but failed. "
        "Please check that the Drive link is set to **'Anyone with the link'** "
